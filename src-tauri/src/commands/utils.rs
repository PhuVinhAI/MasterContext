...
pub fn get_jules_prompt(app: &tauri::AppHandle) -> String {
    let resource_dir = match app.path().resource_dir() {
        Ok(dir) => dir,
        Err(_) => return String::new(),
    };
    let path1 = resource_dir.join("resources").join("jules.md");
    let path2 = resource_dir.join("_up_").join("resources").join("jules.md");
    let path3 = resource_dir.join("jules.md");
    let path4 = std::path::PathBuf::from("../resources").join("jules.md");
...
pub fn build_always_apply_text(
    ...
    if append_jules {
        let jules_prompt = get_jules_prompt(app);
        if !jules_prompt.is_empty() {
            if !final_text.is_empty() {
                final_text.push_str("\n\n");
            }
            final_text.push_str(&jules_prompt);
        }
    }
    if final_text.is_empty() {
        None
...
pub fn perform_auto_export(
    ...
    let final_always_apply_text = build_always_apply_text(
        ...
        data.append_jules_prompt.unwrap_or(false),
    );
